/**********************************************************************************
	Program to recognise numbers, by usage of image processing and computer vision algorithms
	...
	for (int im_num = 1; im_num < amount_of_files + 1; im_num++)
	{
		...
		else
		{
			...
			*********************************************************************************/
			...
			std::vector<std::vector<Point_ID>> segmented = segment.get_segments(im_input);
			std::vector<int> allIDs = segment.read_ID(segmented);
			Numbers_Detector detector;
			std::vector<std::vector < std::vector<std::vector< Point_XY>>>> table_of_numbers = detector.get_numbers(segmented, allIDs);
			/**********************************************************************************
			...
			*********************************************************************************/
			std::vector < std::vector < std::vector<int >>> numbers_detected = detector.detector(table_of_numbers);
